...
export const validateBookQueryParams = [
    query("title").optional().isString().trim().escape().isLength({min: 1}).withMessage("Title must be a non-empty string"),
    query("author").optional().isString().trim().escape().isLength({min: 1}).withMessage("Author must be a non-empty string"),
    query("genre").optional().isString().trim().escape().isLength({min: 1}).withMessage("Genre must be a non-empty string"),
    query("publishedFrom")
        .optional()
